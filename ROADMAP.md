...
| Proxy: PFAF-Parsing via Regex auf rohem HTML | ✅ Tabellenfelder behoben (`commonName`, `eatableScore`, `medsScore`, `materialScore`, `climateZone`) — PFAF hatte die Übersichtstabelle umgebaut (Label jetzt in `<b>`, Wert jetzt in `<span id="ContentPlaceHolder1_XXX">` statt direkt im `<td>`), wodurch die alten Label-Text-Regexes ins Leere liefen. Umgestellt auf Extraktion über die ASP.NET-Control-`id`s, die deutlich stabiler sind als umgebendes Markup, weil sie am Server-Code hängen statt am Seiten-Styling. Physical-Description-Block, Sonne/Wasser-Icons und Funktions-Tags (`boots*`-Divs) waren nicht betroffen. Verifiziert an zwei gespeicherten Seiten (Malus domestica, Robinia pseudoacacia). **Live-Nachtest zeitweise blockiert:** PFAF lieferte gegen Ende dieser Session nur noch leere/verkürzte Seiten an die VPS-IP zurück (~48 statt ~120 KB, unabhängig vom User-Agent) — sieht nach serverseitigem Rate-Limiting aus, ausgelöst durch die vielen Testanfragen in kurzer Zeit während dieser Debug-Session. Fix ist gegen echte Daten geprüft, aber ein Live-Test nach Abklingen der Drosselung steht noch aus. NaturaDB-Parsing-Code existiert noch (`fetchNaturaDb()`), ist aber via `NATURADB_ENABLED = false` deaktiviert (siehe „Lizenz & Datenquellen" oben) |
