...
- [ ] **NaturaDB-Scraping rechtlich klären** — `netlify/functions/plant-proxy.mts` scraped `naturadb.de` ohne erkennbare Lizenz für die redaktionelle Pflanzendatenbank (Nutzungsbedingungen regeln nur nutzergenerierte Inhalte). `robots.txt` enthält einen expliziten `User-agent: Datenbank Crawler` → `Disallow: /`-Eintrag als klares Anti-Scraping-Signal, Betreiber ist die kommerzielle Maseto GmbH. Ohne Lizenz greift das deutsche Datenbankherstellerrecht (§ 87a ff. UrhG) — Abmahnrisiko real, besonders da der Scraping-Code öffentlich auf GitHub liegt. Vor Launch: entweder aktiv Erlaubnis/API-Zugang bei NaturaDB einholen, oder die Quelle deaktivieren/entfernen
- [ ] **PFAF-Attribution nachrüsten** — Datenbank-Texte stehen unter CC BY 4.0 (Namensnennung Pflicht, robots.txt erlaubt den gescrapten Pfad). Attribution existiert bisher nur in der App-internen UI (Badges, Hilfe-Seite), nicht auf den exportierten/gedruckten Karten (Poly-/Streifen-/Baumscheibenkarte) — dort müsste sie eigentlich stehen, da das der tatsächliche „Veröffentlichungs"-Ort der Daten ist
