...
- [x] **Gartenplan: 3D-Ansicht mit vollem Editing** — neuer 2D/3D-Umschalter in Phase B (Toggle, kein Splitscreen — beide Ansichten bearbeiten dieselben `editing.placements`). Three.js per dynamischem `import()` nachgeladen (gleiches Lazy-Chunk-Muster wie `pdf-lib`), neues Modul `gartenplan-3d.ts`. Bäume/Sträucher/Kräuter als prozedurale Formen (Stamm+gejitterte Icosahedron-Krone / gestauchte Icosahedron / flacher Zylinder), Farben identisch zu `LAYER_STYLE`, Größe über dasselbe `displayRadiusM()`. Platzieren/Ziehen direkt in 3D per Raycasting: eigener `pointerdown`-Listener in der **Capture-Phase** auf dem Container (läuft garantiert vor `OrbitControls`' eigenem Listener auf dem Canvas, verifiziert im `OrbitControls`-Quellcode) — bei Treffer auf eine Pflanze wird `controls.enabled=false` gesetzt (OrbitControls prüft das als Erstes und reagiert dann gar nicht erst), sonst läuft die Kamera-Steuerung unverändert. `pointInPolygon` und `seededRandom` aus den bestehenden 2D-Dateien exportiert/extrahiert (`gartenplan-geometry.ts`, `blob-shape.ts`) statt dupliziert. Boundary-Fläche per `ShapeUtils.triangulateShape` trianguliert, `DoubleSide`-Material statt Winding-Erkennung (Klick-Reihenfolge beim Zeichnen ist beliebig). Per Playwright verifiziert: WebGL-Context lädt (funktioniert im Headless-Sandbox ohne Sonderflags), Platzieren in 3D funktioniert, mehrfaches Umschalten 2D↔3D erzeugt keinen Canvas-/Kontext-Leak. **Nicht mehr in dieser Session getestet** (Zeitlimit): Ziehen einer platzierten Pflanze in 3D, Touch-Bedienung in 3D, Speichern+Neuladen eines 3D-bearbeiteten Plans — vor Nutzung einmal gegenprüfen.
