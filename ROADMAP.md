...
- [ ] **Gartenplan: Backup/Restore-Integration** — `gardenPlans` fehlt noch in `sync.ts`s `buildBackupJson()` und den ~6 Restore-Pfaden in `settings.astro` (WebDAV/Gist/Datei/Drag&Drop-JSON). Bewusst zurückgestellt beim MVP-Bau, aber ein echtes Datenverlust-Risiko, sobald das Feature genutzt wird — als eigener kleiner Schritt nachziehen.
...
- [ ] **Gartenplan: Auto-Platzierung / Vorschläge** — `compatScore()` aus `polykulturen.astro` könnte später für "guter Platz für X"-Vorschläge wiederverwendet werden, aktuell nicht angebunden.
