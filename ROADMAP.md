...
- [x] **Offenes Proxy-Relay geschlossen** — `plant-proxy.mts` hatte `Access-Control-Allow-Origin: *` und kein Rate-Limiting; jede fremde Website konnte die Funktion als kostenlosen Scraping-Relay für PFAF zweckentfremden. Jetzt: Origin-Allowlist (`isAllowedOrigin()` — nur `*.netlify.app` inkl. Deploy-Previews, `localhost`/`127.0.0.1`, sowie Requests ganz ohne `Origin`-Header/server-to-server; alles andere bekommt `403`), plus echtes von Netlify durchgesetztes Rate-Limiting über `config.rateLimit` (30 Requests/IP/60s — im Gegensatz zu einem In-Process-Zähler funktioniert das auch über Cold-Starts/mehrere Instanzen hinweg zuverlässig). Mit esbuild kompiliert und die Origin-Logik gegen erlaubte/verbotene/fehlende Origins sowie OPTIONS-Preflight verifiziert
...
| Netlify-Proxy kein Rate-Limiting / offenes CORS | ✅ behoben — Origin-Allowlist + `config.rateLimit` (30/IP/60s), siehe „Lizenz & Datenquellen" oben |
