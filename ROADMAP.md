...
| Proxy: PFAF-Parsing via Regex auf rohem HTML **ist bereits kaputt** | offen, dringlicher als bisher angenommen — beim Testen der Server-Migration festgestellt, dass PFAF sein Seitenlayout geändert hat (`<b>Common Name</b>` statt der erwarteten `<td>`-Tabellenstruktur); die meisten Felder kommen aktuell schon leer zurück, nicht nur „irgendwann in Zukunft, falls sich was ändert". NaturaDB-Parsing-Code existiert noch (`fetchNaturaDb()`), ist aber via `NATURADB_ENABLED = false` deaktiviert (siehe „Lizenz & Datenquellen" oben) |
